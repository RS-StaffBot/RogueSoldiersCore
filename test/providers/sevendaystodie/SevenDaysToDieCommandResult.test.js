...
function createResult(overrides = {}) {
    return new SevenDaysToDieCommandResult({
        ...
        eventLines: ["2026-07-27T22:12:24 INF Time: 51.86m FPS: 20.00"],
...
test("exports frozen command status and completion contracts", () => {
    ...
    assert.equal(Object.isFrozen(SevenDaysToDieCommandCompletionReason), true);
    assert.deepEqual(
...
test("rejects invalid command result values", () => {
    assert.throws(() => createResult({ command: " gettime " }), /command must be a non-empty trimmed string/);
    assert.throws(() => createResult({ status: "UNKNOWN" }), /command status is invalid/);
    assert.throws(() => createResult({ completionReason: "UNKNOWN" }), /completion reason is invalid/);
    assert.throws(() => createResult({ responseLines: [null] }), /response lines are invalid/);
    assert.throws(() => createResult({ eventLines: "not-an-array" }), /event lines are invalid/);
    assert.throws(() => createResult({ startedAt: "not-a-date" }), /start timestamp is invalid/);
    assert.throws(
        () => createResult({ completedAt: "2026-07-27T22:12:18.000Z" }),
        /completion cannot precede its start/
    );
    assert.throws(() => createResult({ truncated: "false" }), /truncation state must be boolean/);
});
...
test("stores response lines separately from unsolicited event lines", () => {
    const result = createResult({
        ...
        eventLines: ["2026-07-27T22:11:54 INF Time: 51.36m FPS: 20.00"]
    });
...
test("keeps captured Telnet evidence sanitized and available", () => {
    ...
    assert.equal(combined.includes("0002c60901644d5dbbe98aa9575f6d65"), false);
