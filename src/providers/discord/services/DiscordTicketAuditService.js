...
class DiscordTicketAuditService {
    ...
    constructor({ recordingService } = {}) {
        if (
            ...
            throw new Error(
                "Discord Ticket audit recording boundary is invalid."
            );
        }
    ...
    resolveAction(action) {
        if (!SUPPORTED_ACTIONS.includes(action)) {
            throw new Error(
                "Discord Ticket audit action is invalid."
            );
        }

        return `ticket.staff.${action}`;
    }
...
module.exports = DiscordTicketAuditService;
